/*=================================================================
  ...
  13Jun16 Create boxText style by setting TEXT Cobj bgFillColor ARC
  12Mar17 Clean up paintPath gradient generators
          render parmaters passed in an object
          Removed buffered drawing, not needed since FF fixed   ARC
  13Mar17 Converted clipPath to use options object as parameter ARC
  14Mar17 Added Path.bBoxCmds to support text drawn as a Path   ARC
  16Mar17 Replace Cobj with Path, Shape, Img and Text objects   ARC
  17Mar17 FF bugWorkAround: clip needs a fill command to work   ARC
  19Mar17 DrawPath etc methods no longer return the object      ARC
  20Mar17 Remove all references to Cobj
          Reuse Path constructor for Shape                      ARC
  28Mar17 Released as Cango-9v00                                ARC
  02Apr17 Move draw method x,y coords into options object
          Give draw methods support for scl and degs options    ARC
  03Apr17 bugfix: toImgObj not firing complete event use load
          bugfix: renderOpts should be properties of object not
           Cango so they survive Img load callback
          bugfix: setting Shape.iso false not working           ARC
  04Apr17 Simplify renderOptions assignment                     ARC
  09Apr17 Give shapeDefs.cross options height parameter
          Remove arrow and arrowArc from shapeDefs              ARC
  =================================================================*/
...
(function() {
  ...
  svgParser = (function ()
  {
        ...
        svgProtocol = {
          ...
          "A" : {      // Circular arc
            ...
            addXYoffset: function(curr, xOfs, yOfs){
              ...
              x += xOfs;
              y += yOfs;
              return ["A", rx, ry, xrot, lrg, swp, x, y];
            },
            invertCoords: function(curr){
              var rx = curr[1],
                  ry = curr[2],
     ...
     * one of the keys in the SVG proptocol. If a bad cmd
     * is found an empty array is returned. If no bad
  ...
  if (shapeDefs === undefined)
  {
    shapeDefs = {
      ...
      'cross': function(width, height){
                  var w = width || 1;
                  var h = height || w;
                  return ['m', -0.5*w, 0, 'l', w, 0, 'm', -0.5*w, -0.5*h, 'l', 0, h];},
  ...
  function appendPathFn(obj, delMove)
  {
  ...
  Path = function(commands, options)
  {
    ...
    this.bBoxCmds = [];             // DrawCmd array for the svg text, text or img bounding box
    ...
    this.fillCol = null;            // here for Shape only, not used by Path
    this.strokeCol = null;          // renderer will stroke a path and shape outline in this color
    this.lineWidth = null;          // in pixels will not scale with xscl changes or obj.scale()
    ...
    this.dragNdrop = null;
    // properties set by setProperty if undefined render uses Cango default
    this.border = false;            // here for Shape only Path's don't have borders
    ...
    this.dashed = null;
    this.dashOffset = 0;
    this.renderOptions = {x:0, y:0, scl:1, degs:0};
  ...
  Path.prototype.appendPath = appendPathFn;
  Path.prototype.revWinding = revWindingFn;
  ...
  Shape = function(commands, options)
  {
    ...
    objClass = new Path(commands, options);
    objClass.type = "SHAPE";
    // only difference is the default value for 'iso' property
    if (opt.hasOwnProperty("iso"))
    {
      objClass.setProperty("iso", opt["iso"]);
    }
    else if (opt.hasOwnProperty("isotropic"))
    {
      objClass.setProperty("isotropic", opt["isotropic"]);
    }
    else
    {
      objClass.setProperty("iso", true);   // default true = maintain aspect ratio
    }
  ...
  Text = function(txtString, options)
  {
    ...
    this.shadowBlur = 0;
    this.shadowColor = "#000000";
    // bgFillColor to create box text for Text only
    this.bgFillColor = "rgba(0,0,0,0.0)";
    this.renderOptions = {x:0, y:0, scl:1, degs:0};
...
//===============================================================================
  ...
  Cango = function(canvasId)
  {
    ...
    function resizeLayers()
    {
      ...
      // kill off any animations on resize (else they contiune running along with any new ones)
    ...
    this.heightPW = this.widthPW/this.aRatio;    // height of canvas in Percent Width Coords
    ...
    this.yDown = false;   // set by setGridboxRHC & setGridboxSVG to signal coord system to setWorldCoords
    this.vpW = this.rawWidth;               // gridbox width in pixels (default to full canvas size)
    this.vpH = this.rawHeight;              // gridbox height in pixels
    this.vpOrgX = 0;                        // gridbox lower left from canvas left in pixels
    this.vpOrgY = this.rawHeight;           // gridbox lower left from canvas top
  ...
  {
    // gridbox origin is lower left for Cartesian, upper left for SVG
    var vpOrgXWC = vpOriginX || 0,  // gridbox origin x world coord
        vpOrgYWC = vpOriginY || 0;  // gridbox origin y world coord
  ...
  Cango.prototype.render = function(pathObj, options)
  {
    var savThis = this,
        prop,
        opt = (typeof options === 'object')? options: {};   // avoid undeclared object errors
    ...
    function processCobj(obj)
    {
      ...
      obj.renderOptions.x = opt.x || 0;
      obj.renderOptions.y = opt.y || 0;
      obj.renderOptions.scl = opt.scl || 1;
      obj.renderOptions.degs = opt.degs || 0;
    ...
    if (isArray(pathObj))
    {
      flatten(pathObj).forEach(processCobj);  // handle nested arrays of objects
    }
    else if (pathObj)
    {
      processCobj(pathObj);
  ...
  Cango.prototype.paintImg = function(pathObj)
  {
    ...
    var savThis = this,
        ...
        xPos = pathObj.renderOptions.x,
        yPos = pathObj.renderOptions.y,
        scale = pathObj.renderOptions.scl*pathObj.imgXscale,  // imgXscale is from obj.scale (permanent)
        degs = pathObj.renderOptions.degs,
        currLr, aidx;
  ...
  Cango.prototype.paintPath = function(pathObj)
  {
    ...
    var savThis = this,
        ...
        xPos = pathObj.renderOptions.x,
        yPos = pathObj.renderOptions.y,
        scale = pathObj.renderOptions.scl,
        degs = pathObj.renderOptions.degs,
        j,
    ...
    function genLinGrad(lgrad)
    {
      var p1x = lgrad.grad[0],
          ...
          p2x = lgrad.grad[2],
          p2y = lgrad.grad[3],
          grad = savThis.ctx.createLinearGradient(xsl*p1x, ysl*p1y, xsl*p2x, ysl*p2y);
    ...
    function genRadGrad(rgrad)
    {
      var p1x = rgrad.grad[0],
          ...
          p2y = rgrad.grad[4],
          r2 = rgrad.grad[5],
          grad = savThis.ctx.createRadialGradient(xsl*p1x, ysl*p1y, xsl*r1, xsl*p2x, ysl*p2y, xsl*r2);
    ...
    if (pathObj.type === "SHAPE")      // if a SHAPE, fill with color
    {
      ...
      {
        this.ctx.fillStyle = genLinGrad(col);
      }
      else if (col instanceof RadialGradient)
      {
        this.ctx.fillStyle = genRadGrad(col);
      }
      else
      {
  ...
  Cango.prototype.paintText = function(pathObj)
  {
    var savThis = this,
        A = 0,
        sinA = 0,
        ...
        degs = pathObj.renderOptions.degs,
        currLr, aidx,
        fntSz = this.xscl*scale*pathObj.fontSizeWC, // use xscl to support zoom & pan
        // if obj.fontWeight or fontSize undefined use Cango default
    ...
    if (pathObj.bgFillColor != "rgba(0,0,0,0.0)")
    {
      // create the bounding box path
      this.ctx.save();
      this.ctx.beginPath();
      pathObj.bBoxCmds.forEach(function(bbCmd){
        savThis.ctx[bbCmd.drawFn].apply(savThis.ctx, bbCmd.parmsPx);
      });
      this.ctx.fillStyle = pathObj.bgFillColor;
      this.ctx.strokeStyle = pathObj.bgFillColor;
    ...
    // now set up the canvas to fillText
    this.ctx.save();   // save current context

    // handle drop shadows prior to fillText
    this.dropShadow(pathObj);
    ...
    this.ctx.fillText(pathObj.drawCmds,
                      ...
                      -this.xscl*scale*(pathObj.imgY+pathObj.imgLorgY));
    // text borders are around the letters even for boxText
    if (pathObj.border)
    {
  ...
  Cango.prototype.drawPath = function(path, options)
  {
    var opts = options || {},   // avoid properties of undefined error
        pathObj = new Path(path, opts);
    this.render(pathObj, {x:opts.x, y:opts.y, scl:opts.scl, degs:opts.degs});
  };

  Cango.prototype.drawShape = function(path, options)
  {
    var opts = options || {},   // avoid properties of undefined error
        shapeObj = new Shape(path, opts);
    this.render(shapeObj, {x:opts.x, y:opts.y, scl:opts.scl, degs:opts.degs});
  };

  Cango.prototype.drawText = function(str, options)
  {
    var opts = options || {},   // avoid properties of undefined error
        txtObj = new Text(str, opts);
    this.render(txtObj, {x:opts.x, y:opts.y, scl:opts.scl, degs:opts.degs});
  };

  Cango.prototype.drawImg = function(imgURL, options)
  {
    var opts = options || {},   // avoid properties of undefined error
        imgObj = new Img(imgURL, opts);
    this.render(imgObj, {x:opts.x, y:opts.y, scl:opts.scl, degs:opts.degs});
  };

  Cango.prototype.clipPath = function(pathObj, options)
  {
    // used for type: PATH, SHAPE
    var savThis = this,
        prop,
  ...
  Cango.prototype.dupCtx = function(src_graphCtx)
  {
    ...
    this.xoffset = src_graphCtx.xoffset;  // world x origin offset from gridbox left in pixels
    this.yoffset = src_graphCtx.yoffset;  // world y origin offset from gridbox bottom in pixels
    ...
    this.renderOptions = clone(src_graphCtx.renderOptions);
  };
  ...
  Cango.prototype.toImgObj = function(obj)
  {
    ...
    imgObj = new Img(gc.cnvs.toDataURL());  // toDataURL is synchronous but slow
